...
### Python <Badge type="warning" text="experimental" />
...
A lightweight MySQL 8.4+ CDC (Change Data Capture) engine extracted from MygramDB. Parses MySQL binlog replication events and provides a streaming API for applications. Supports MySQL 9.x (VECTOR type, RSA auth). Node.js (N-API) and Python (ctypes) bindings.
